...
export interface QueryResult<TResult, TVariables> {
  data: null | TResult;
  status: 'success' | 'loading' | 'error'; // SWYX
  error: null | Error;
  isLoading: boolean;
  isFetching: boolean;
  isCached: boolean;
...
export interface MutationOptions {
    ...
    onSuccess(): void
    // there are others i didnt touch
}
export namespace queryCache {
  export function refetchQueries(key: string): void; // swyx
}
...
export interface MutationResult<TResults> {
  data: TResults | null;
  status: 'idle' | 'success' | 'loading' | 'error'; // SWYX
  isLoading: boolean;
  error: null | Error;
